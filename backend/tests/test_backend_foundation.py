...
def test_fastapi_entrypoint_uses_router_wiring() -> None:
    ...
    config_source = (BACKEND_APP / "core" / "config.py").read_text(encoding="utf-8")

    assert "def create_app() -> FastAPI:" in main_source
    assert "app = create_app()" in main_source
    assert "settings = get_settings()" in main_source
    assert "app.state.settings = settings" in main_source
    ...
    assert "def load_settings(environ: Mapping[str, str] | None = None) -> Settings:" in config_source
    assert "def get_settings() -> Settings:" in config_source
...
def test_workers_delegate_to_domain_services() -> None:
    ...
    assert "settings = get_settings()" in celery_source
    assert "broker=settings.redis_url" in celery_source
    assert "backend=settings.celery_result_backend" in celery_source
