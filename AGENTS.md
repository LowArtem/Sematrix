...
- Keep note-editor inline image rendering derived from `assetId` inside that same centralized image extension, so reloaded notes still resolve `/api/assets/{assetId}` even when stored `content_json` omits a persisted `src` attribute.
